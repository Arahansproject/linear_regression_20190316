...
class Kmean():
    def execute(self):
        ...
        # 센트로이드는 D0 차원의 크기가 4개이고, D1 차원은 vectors 동일한 2인 행렬

        expended_vectors = tf.expand_dims(vectors, 0)  # 두 텐서에 차원을 추가
        expended_centroids = tf.expand_dims(centroids, 1) # 두 텐서를 2차원에서 3차원으로 만들어 뺄셈이 가능하도록 함
